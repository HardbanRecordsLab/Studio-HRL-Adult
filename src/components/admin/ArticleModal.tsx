...
import { X, Save, Upload, FileText, Clock, Type, Layout, Tag as TagIcon, Globe } from 'lucide-react';
...
interface AcademyBlogArticle {
  ...
  title: string;
  slug: string;
  category: 'Psychologia' | 'Platformy' | 'Live Cam' | 'Bezpieczeństwo' | 'Monetyzacja' | 'Marka' | 'Marketing' | 'Biznes' | 'Technologia';
  tag: 'MUST READ' | 'ANALIZA' | 'PORADNIK' | 'BEZPIECZEŃSTWO' | 'ZAAWANSOWANY' | 'STRATEGIA' | 'PODSTAWY' | 'TECH';
  excerpt: string;
  content: string;
  readTime: string;
  thumbnail?: string;
...
const ArticleModal: React.FC<ArticleModalProps> = ({
  ...
  const categories: AcademyBlogArticle['category'][] = [
    'Psychologia', 'Platformy', 'Live Cam', 'Bezpieczeństwo',
    'Monetyzacja', 'Marka', 'Marketing', 'Biznes', 'Technologia'
  ];

  const tags: AcademyBlogArticle['tag'][] = [
    'MUST READ', 'ANALIZA', 'PORADNIK', 'BEZPIECZEŃSTWO',
          ...
          <div className="flex items-center justify-between p-10 border-b border-white/5 bg-white/[0.02]">
            <div>
               <h2 className="text-3xl font-bold italic font-georgia text-[#c9a84c]">
                 {article ? 'Edycja <span className="text-white">Artykułu</span>' : 'Nowy <span className="text-white">Artykuł</span>'}
               </h2>
               <p className="text-[9px] text-gray-500 uppercase tracking-[4px] font-black mt-2">HRL Academy Production Unit</p>
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="p-10 overflow-y-auto max-h-[calc(90vh-250px)] custom-scrollbar">
            <div className="grid grid-cols-12 gap-10">
               ...
               <div className="col-span-12 lg:col-span-8 space-y-8">
                  ...
                  {/* Excerpt */}
                  <div className="space-y-3">
                    <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest ml-2">Krótki Opis (Excerpt)</label>
                    <textarea
                  ...
                  <div className="space-y-3">
                    <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest ml-2 flex justify-between items-center">
                       ...
                       <span className="text-[8px] opacity-40 italic lowercase">Wspiera tagi &lt;br/&gt;, **bold**, *italic*</span>
                    </label>
                    <textarea
               ...
               <div className="col-span-12 lg:col-span-4 space-y-8">
                  ...
                  <div className="bg-white/[0.02] border border-white/5 rounded-3xl p-6 space-y-6">
                    ...
                    <div className="space-y-3">
                      ...
                      >
                        {tags.map(tag => (
                          <option key={tag} value={tag} className="bg-black">{tag}</option>
                        ))}
                      </select>
                    </div>
                  </div>
                  ...
                  <div className="bg-[#c9a84c]/5 border border-[#c9a84c]/10 rounded-3xl p-6 space-y-4">
                     <div className="space-y-3">
                        <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Czas Czytania</label>
                  ...
                  <div className="space-y-4">
                    <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest ml-2">Miniatura Artykułu</label>
                    <CldUploadWidget
                        ...
                        <div className="space-y-4">
                          ...
                          >
                            ...
                            <span className="text-[8px] font-black text-gray-500 uppercase tracking-widest group-hover:text-white">
                               {formData.thumbnail ? 'Zmień Grafikę' : 'Wgraj Miniaturę'}
                            </span>
                          </button>
                          {formData.thumbnail && (
                  ...
                  <div className="flex items-center justify-between p-6 bg-black border border-white/5 rounded-3xl">
                    <div className="flex items-center gap-4">
                      <div className={cn("w-10 h-10 rounded-2xl flex items-center justify-center transition-all", formData.isPublished ? "bg-green-500/10 text-green-500" : "bg-gray-800/10 text-gray-600")}>
                         <FileText className="w-5 h-5" />
                      </div>
                      <div>
                        <div className="text-[9px] font-black text-white uppercase tracking-widest">Publikacja</div>
                        <div className="text-[7px] text-gray-600 uppercase tracking-tight mt-0.5">
                          {formData.isPublished ? 'Live na portalu' : 'Zapisz jako szkic'}
                        </div>
                      </div>
                    </div>
            ...
            <div className="flex items-center justify-end gap-6 mt-12 pt-8 border-t border-white/5">
              ...
              >
                ...
                {loading ? 'Przetwarzanie...' : (article ? 'Aktualizuj Artykuł' : 'Stwórz Artykuł')}
              </button>
            </div>
          </form>
        </motion.div>
